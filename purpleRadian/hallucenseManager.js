...
//  This is the "Purple Radian" variation.
//
...
(function(){ 
    ...
    var NBR_PARTICULATIONS = 13;
    
    var PARTICULATIONS_MAX_RATE = 800;
    var PARTICULATIONS_MAX_LIFE = 4;
    ...
    function getMagicColor() {
        var range = 0.16; //60
        var target = 0.83; //300
        
        var hue = target + ((Math.random() * range * 2) - range);
        if (hue < 0) {
            hue = hue + 1;
        }
    ...
    function getHallucenceproperties(distance) {
        ...
        var colorStart = hslToRgb(getMagicColor(), 1, 0.5);
        var color = hslToRgb(getMagicColor(), 1, 0.5);
        var colorEnd = hslToRgb(getMagicColor(), 1, 0.5);
